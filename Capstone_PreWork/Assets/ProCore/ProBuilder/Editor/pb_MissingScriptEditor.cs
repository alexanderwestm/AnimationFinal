...
namespace ProBuilder2.EditorCommon
{
	...
	#region Members
		...
		{
			if(skipEvent && UnityEngine.Event.current.type == UnityEngine.EventType.Repaint)
			{
				skipEvent = false;
				return;
			}
			...
			if((doFix && UnityEngine.Event.current.type == UnityEngine.EventType.Repaint) || GUILayout.Button("Reconnect"))
			{
